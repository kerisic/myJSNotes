...
function addNewNoteListener() {
  ...
  submit.addEventListener("click", function (event) {
    ...
    var note = new Note(noteID, content.value); // create a new Note object
...
function deleteNote(e) {
  if (e.target.classList.contains("delete-note-btn")) {
    // if the event target's class contains 'delete-note-btn'
...
function showNote(noteID) {
  // show the body of the note by retrieving the right object from notes
  let notes = getDataFromStorage();
  ...
  fetch("https://makers-emojify.herokuapp.com/", {
    ...
    .then((jsonresponse) => {
      document.getElementById(
        "note"
      ).innerHTML = `${jsonresponse.emojified_text}`;
    });
}
